document.addEventListener("DOMContentLoaded", () => {
  ...
  const elements = {
    ...
    realTimeChartCtx: document
      .getElementById("real-time-chart")
      .getContext("2d"), // Real-time chart context
    experimentChartCtx: document
      .getElementById("experiment-chart")
      .getContext("2d"), // Experiment chart context
    derivativeChartCtx: document
      .getElementById("derivative-chart")
      .getContext("2d"), // Derivative chart context
    ...
    addExperimentDataButton: document.getElementById(
      "add-experiment-data-button"
    ), // Add data to experiment button
    downloadRealTimeDataButton: document.getElementById(
      "download-real-time-data-button"
    ), // Download real-time data button
    downloadExperimentDataButton: document.getElementById(
      "download-experiment-data-button"
    ), // Download experiment data button
    downloadDerivativeDataButton: document.getElementById(
      "download-derivative-data-button"
    ), // Download derivative data button
    ...
    realTimeTable: document.querySelector("#real-time-table-body").parentElement
      .parentElement, // Real-time table element
    experimentTable: document.querySelector("#experiment-table-body")
      .parentElement.parentElement, // Experiment table element
  };
  ...
  let realTimeData = [],
    experimentData = [],
    derivativeData = []; // Data arrays
  let volumeSum = 0,
    readCount = 0,
    experimentReadCount = 0; // Counters
  let isConnected = false; // Connection state

  // Initialize Charts
  const charts = {
    realTimeChart: new Chart(
      elements.realTimeChartCtx,
      createChartConfig("Real-Time Chart", "Read Number", "p(H2) /ppmv")
    ),
    experimentChart: new Chart(
      elements.experimentChartCtx,
      createChartConfig("Experiment Chart", "Volume", "Pressao /")
    ),
    derivativeChart: new Chart(
      elements.derivativeChartCtx,
      createChartConfig(
        "Derivative Chart",
        "Average Volume",
        "Derivative",
  ...
  elements.downloadRealTimeDataButton.addEventListener("click", () =>
    downloadCSV(realTimeData, "real-time_data.csv", [
      "time",
      "read",
      "reading",
      "temperature",
  ...
  elements.downloadExperimentDataButton.addEventListener("click", () =>
    downloadCSV(experimentData, "experiment_data.csv", [
      "time",
      "read",
      "volume",
      "reading",
  ...
  elements.downloadDerivativeDataButton.addEventListener("click", () =>
    downloadCSV(derivativeData, "derivative_data.csv", [
      "averageVolume",
      "derivativeValue",
    ])
  );
  ...
  async function connect() {
    ...
    try {
      ...
      alert(
        "Failed to connect to the instrument. Please check the connection and try again."
      );
    }
  }
  ...
  // Start reading data from the serial port  - 100 ms
  function startSerialReading() {
    readTimer = setInterval(readSerialData, 1); // Set an interval to read data every 1ms
  }
  ...
  async function readSerialData() {
    try {
      ...
      console.log("lendo - linha 216");
      buffer += new TextDecoder().decode(value); // Append new data to the buffer
      let index;
      //console.log(buffer);
      /*switch (elements.instrumentSelect.name) {
        case "AS7341-FIA":
          */
      //console.log(buffer, "lendo - linha 216");
      while ((index = buffer.indexOf("\r")) >= 0) {
        // Process each line of data
        /* luca 201 tem o formato xxxxx, xxx  Potencial, temperatura ou pH, temperatura */
        //const dataStr = buffer.slice(0, index + 1).trim(); // Extract a single line of data
        const dataStr = buffer;

        buffer = buffer.slice(index + 1); // Remove processed data from buffer
        /*index=buffer.indexOf(','); //posição do 1o delimitador
      const dataStr = buffer.slice(0, index + 1).trim(); // Extract a first data in the line 
      */
        const parsedData1 = parseData(dataStr); // Parse the data
        if (parsedData1) lastValidData = parsedData1; // Update last valid data
        //console.log("linha 212", lastValidData);
      }
    } catch (err) {
  ...
  function updateRealTimeChart() {
    ...
    charts.realTimeChart.data.datasets[0].data = recentData.map((data) => ({
      x: data.read,
      y: data.leitura,
    })); // Update chart data
  ...
  function updateRealTimeData() {
    ...
    //
    //console.log("linha 250",realTimeData);

    //
    updateTable(elements.realTimeTableBody, realTimeData, [
      "time",
      "read",
      "leitura",
      "temperature",
  ...
  // Add experiment data to the chart  Titration DATA and table
  function addExperimentData() {
    ...
    if (experimentData.length === 0) {
      data = {
        ...lastValidData,
        ...currentDateTime,
        read: ++experimentReadCount,
        volume: 0,
      }; // Create new data point with volume 0
    ...
    /*console.log(
      ...
      experimentData,
      elements.experimentTableBody
    );*/

    updateTable(elements.experimentTableBody, experimentData, [
      "time",
      "read",
      "volume",
      "leitura",
    ]); // Update experiment table
    updateChart(charts.experimentChart, experimentData, "volume", "reading"); // Update experiment chart
  ...
  function parseData(dataStr) {
    ...
    let leitura;
    let temperature;
    //console.log("311", delimiter);
    if (delimiter == -1) {
      leitura = parseInt(dataStr);
      temperature = parseInt(dataStr);
    } else {
      const parts = dataStr.split(","); // Split data string into parts
      /*if (parts.length !== 2) return null; // Ensure data is valid
       */
      leitura = parseFloat(parts[0]); // Parse MQ-8 analógico
      temperature = parseFloat(parts[1]); // Parse pressão
      sensorH2 = parseFloat(parts[0]).toFixed(1); // Parse and format temperature
      console.log(dataStr, leitura, temperature, sensorH2);
      /*if (isNaN(pH) || pH < 1 || pH > 14 || isNaN(temperature)) return null; // Validate
       */
      //console.log("325", leitura, temperature);
    }
    return { leitura, temperature }; // Return parsed data
    //return leitura;
  }
  ...
  function createTableRow(data, fields) {
    return `<tr>${fields
      .map((field) => `<td>${data[field]}</td>`)
      .join("")}</tr>`; // Generate HTML for a table row
  }
  ...
  function downloadCSV(dataArray, filename, headers) {
    const csvContent =
      ...
      [
        headers.join(","),
        ...dataArray.map((e) => headers.map((header) => e[header]).join(",")),
  ...
  function playBipSound() {
    const audioContext = new (window.AudioContext ||
      window.webkitAudioContext)(); // Create audio context
